...
func trainCmd(args []string) {
    ...
    if *in == "" { fmt.Fprintln(os.Stderr, "--in is required"); os.Exit(1) }
    ...
    if err := model.SaveJSON(*out); err != nil { fmt.Fprintln(os.Stderr, err); os.Exit(1) }
}
...
func predictCmd(args []string) {
    ...
    if *in == "" || *modelPath == "" { fmt.Fprintln(os.Stderr, "--in and --model are required"); os.Exit(1) }
    ...
    if *asCSV {
        ...
        cw.Flush()
        if err := cw.Error(); err != nil { fmt.Fprintln(os.Stderr, err); os.Exit(1) }
        return
    }
    ...
    for _, it := range items {
        ...
        if *proba { out["proba"] = model.PredictProba(it) }
        if err := enc.Encode(out); err != nil { fmt.Fprintln(os.Stderr, err); os.Exit(1) }
    }
    bw.Flush()
}
...
func visualizeCmd(args []string) {
    ...
    if *modelPath == "" { fmt.Fprintln(os.Stderr, "--model is required"); os.Exit(1) }
    model, err := dtree.LoadJSON(*modelPath)
    if err != nil { fmt.Fprintln(os.Stderr, err); os.Exit(1) }
    if err := model.ToHTML(*outHTML); err != nil { fmt.Fprintln(os.Stderr, err); os.Exit(1) }
    if *outDOT != "" {
        if err := os.WriteFile(*outDOT, []byte(model.ToDOT()), 0644); err != nil { fmt.Fprintln(os.Stderr, err); os.Exit(1) }
    }
}
...
func readTrainingSet(path, format, label string) (dtree.TrainingSet, error) {
    ...
    // ensure label exists
    for _, it := range items {
        if _, ok := it[label]; !ok { return nil, fmt.Errorf("missing label '%s' in some rows", label) }
    }
...
func readItems(path, format, label string) ([]dtree.TrainingItem, []string, error) {
    ...
    case "jsonl":
        ...
        for sc.Scan() {
            ...
            if err := json.Unmarshal(sc.Bytes(), &m); err != nil { return nil, nil, err }
            items = append(items, dtree.TrainingItem(m))
        }
        if err := sc.Err(); err != nil { return nil, nil, err }
        // collect headers from first item (best-effort)
        hdr := []string{}
        if len(items) > 0 {
            for k := range items[0] { hdr = append(hdr, k) }
        }
